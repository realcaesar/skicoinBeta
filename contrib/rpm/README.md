...
reference the release tarball available from https://skicoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/skicoin/skicoin) but are not part of
...
Currently only `Source10` is used, for the example `skicoin.conf` file.
...
in the skicoin git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the skicoin
developers so that an official fix to the issue can make it into the next
release.

### Patch0 skicoin-0.12.0-libressl.patch
...
The `skicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `skicoin-server` package. At this point, most RPM based
...
The `skicoin-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/skicoin/). He has been
...
4. I renamed the `skicoin` package that contains the Qt GUI to `skicoin-core` as
that appears to be how the general population refers to it, in contrast to
`skicoin-xt` or `skicoin-classic`. I wanted to make sure the general population
