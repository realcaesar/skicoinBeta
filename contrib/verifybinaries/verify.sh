...
###   skicoincore.org and skicoin.org and compares them.
...
WORKINGDIR="/tmp/skicoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://skicoincore.org"
HOST2="https://skicoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="skicoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'skicoin-',
...
if [ $? -ne 0 ]; then
   echo "skicoin.org failed to provide signature file, but skicoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "skicoin.org and skicoincore.org signature files were not equal?"
