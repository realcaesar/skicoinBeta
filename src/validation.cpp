...
#if defined(NDEBUG)
# error "Skicoin Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Skicoin Signed Message:\n";
...
// SKICOIN
bool CheckSkicoinInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity)
{
	...
	if (block.vtx.empty() && tx.nVersion == SKICOIN_TX_VERSION) {
		...
		if (good)
		{
			...
			return state.DoS(100, false, REJECT_INVALID, "skicoin-inputs-error");
		...
		if (CreateGraphFromVTX(nHeight, sortedBlock.vtx, graph, vertices, mapTxIndex)) {
			...
			if (!sortedBlock.vtx.empty()) {
				if (!DAGTopologicalSort(sortedBlock.vtx, conflictedIndexes, graph, mapTxIndex)) {
					if (fDebug)
						LogPrintf("CheckSkicoinInputs: Toposort failed");
					return true;
				}
			}
		}
		...
		nFlushIndexBlocks++;
		if ((nFlushIndexBlocks % 200) == 0 && !FlushSkicoinDBs())
			return state.DoS(0, false, REJECT_INVALID, "Failed to flush skicoin databases");
	}
	return true;
}
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun, bool bMultiThreaded)
{
		...
		BOOST_FOREACH(const CTxIn &txin, tx.vin)
		{
			...
			if (itConflicting != pool.mapNextTx.end())
			{
				...
				if (!setConflicts.count(ptxConflicting->GetHash()))
				{
					...
					// SKICOIN txs are not replacable
					else if (ptxConflicting->nVersion == SKICOIN_TX_VERSION) {
						return state.DoS(0, error("AcceptToMemoryPool : Skicoin Transaction %s conflicts with Transaction request %s",
							hash.ToString(), ptxConflicting->GetHash().ToString()),
...

	{
		...
		if (!bMultiThreaded) {
			...
			if (!control.Wait())
				return false;
			if (!CheckSkicoinInputs(tx, state, view, true, chainActive.Height(), CBlock())) {
				return false;
			}
		}
		...
		if (bMultiThreaded && threadpool != NULL)
		{
			...
			static int totalSkicoinCheckCount = 0;
			static int totalSkicoinCheckMicros = 0;
			...
			std::packaged_task<void()> task([&pool, ptx, hash, coins_to_uncache, hashCacheEntry, vChecks]() {
				...
				int thisSkicoinCheckCount = 0;
				int thisCheckMicros = 0;
				int thisSkicoinCheckMicros = 0;
				...
				if (isCheckPassing)
				{
					int64_t skicoinCheckTime;
					if (fLogThreadpool) {
						skicoinCheckTime = GetTimeMicros();
						thisSkicoinCheckCount += 1;
					}
					if (!CheckSkicoinInputs(txIn, validationState, coinsViewCache, true, chainActive.Height(), CBlock()))
					{
						nLastMultithreadMempoolFailure = GetTime();
						LOCK2(cs_main, mempool.cs);
						LogPrint("mempool", "%s: %s\n", "CheckSkicoinInputs Error", hash.ToString());
						BOOST_FOREACH(const COutPoint& hashTx, coins_to_uncache)
							pcoinsTip->Uncache(hashTx);
						pool.removeRecursive(txIn, MemPoolRemovalReason::UNKNOWN);
						pool.ClearPrioritisation(hash);
					...
					if (fLogThreadpool) {
						// how long did we run CheckSkicoinInputs()'s
						thisSkicoinCheckMicros = GetTimeMicros() - skicoinCheckTime;
					}
				}
				if (fLogThreadpool) {
					// metrics
					...
					totalSkicoinCheckCount += thisSkicoinCheckCount;
					totalSkicoinCheckMicros += thisSkicoinCheckMicros;
			...
			if (fLogThreadpool && totalCheckCount > 0 && totalSkicoinCheckCount > 0 && totalExecutionCount > 0 && (!fUnitTest || (fUnitTest && totalExecutionCount % 100 == 0))) {
				int avgCheckMicros = totalCheckMicros / totalCheckCount;
				int avgSkicoinCheckMicros = totalSkicoinCheckMicros / totalSkicoinCheckCount;
				...
				const std::string &messageInternals = "THREADPOOL::%s:Signature check internals - check(%d): total %lld, avg %lld microseconds, skicoin(%d): total %lld, avg %lld microseconds\n";
				...
				LogPrint("threadpool", messageInternals, hash.ToString(), totalCheckCount, totalCheckMicros, avgCheckMicros, totalSkicoinCheckCount, totalSkicoinCheckMicros, avgSkicoinCheckMicros);
...
{
	// SKICOIN if its been less 60 seconds since the last MT mempool verification failure then fallback to single threaded
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
	...
	const CAmount &nSuperblockPart = (nSubsidy*0);
    ...
    {
        if (nHeight >= 0 && nHeight <= 288)
            nSubsidy *= 0.75;
        else if (nHeight >= 289 && nHeight <= 1439)
            nSubsidy *= 0.30;
        else
            nSubsidy *= 0.20;
    }
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
					...
					if (RemoveSkicoinScript(out.scriptPubKey, scriptPubKeyOut))
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
						...
						CScript scriptOut;
						CScript scriptPubKeyOut;
						if (RemoveSkicoinScript(prevout.scriptPubKey, scriptPubKeyOut))
							scriptOut = scriptPubKeyOut;
						else
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
	...
	if (!CheckSkicoinInputs(*block.vtx[0], state, viewOld, fJustCheck, pindex->nHeight, block))
		return error("ConnectBlock(): CheckSkicoinInputs on block %s failed\n",
			block.GetHash().ToString());
    int64_t nTime3 = GetTimeMicros(); nTimeConnect += nTime3 - nTime2;
    LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime3 - nTime2), 0.001 * (nTime3 - nTime2) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime3 - nTime2) / (nInputs-1), nTimeConnect * 0.000001);

    // SKICOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
	...
	if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, nFees, nTotalRewardWithMasternodes)) {
		...
		return state.DoS(0, error("ConnectBlock(SKI): couldn't find masternode or superblock payments"),
			REJECT_INVALID, "bad-cb-payee");
	}

    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, nTotalRewardWithMasternodes, nFees, strError)) {
        return state.DoS(0, error("ConnectBlock(SKI): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    // END SKICOIN
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    if (fDoFullFlush) {
		...
		// SKICOIN
		if (!FlushSkicoinDBs())
			return AbortNode(state, "Failed to flush skicoin databases");
        nLastFlush = nNow;
    }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
			// SKICOIN, skip auxpow bit 8 or dummy bit 28
			if (bit == 8 || bit == 28)
				continue;
...
{
	// SKICOIN check auxpow first then fallback to normal check
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SKI : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(SKI): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // SKICOIN check for version 2, 3 and 4 upgrades
