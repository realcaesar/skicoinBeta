...
#ifndef SKICOIN_VALIDATION_H
#define SKICOIN_VALIDATION_H
...
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 2 * COIN; // "smallest denom" + X * "denom tails"
...
static const int64_t DEFAULT_MAX_TIP_AGE = 2.5 * 6 * 60 * 60; // ~144 blocks behind -> 2 x fork detection time, was 24 * 60 * 60 in skicoin
...
bool CheckSkicoinInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity = false);
...
// SKICOIN add it here so chain.cpp can access
