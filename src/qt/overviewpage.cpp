...
public:
    TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(), unit(SkicoinUnits::SKI),
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(foreground);
        QString amountText = SkicoinUnits::floorWithUnit(unit, amount, true, SkicoinUnits::separatorAlways);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, balance, false, SkicoinUnits::separatorAlways));
    ui->labelUnconfirmed->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedBalance, false, SkicoinUnits::separatorAlways));
    ui->labelImmature->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, immatureBalance, false, SkicoinUnits::separatorAlways));
    ui->labelAnonymized->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, anonymizedBalance, false, SkicoinUnits::separatorAlways));
    ui->labelTotal->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, balance + unconfirmedBalance + immatureBalance, false, SkicoinUnits::separatorAlways));
    ui->labelWatchAvailable->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance, false, SkicoinUnits::separatorAlways));
    ui->labelWatchPending->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, watchUnconfBalance, false, SkicoinUnits::separatorAlways));
    ui->labelWatchImmature->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, watchImmatureBalance, false, SkicoinUnits::separatorAlways));
    ui->labelWatchTotal->setText(SkicoinUnits::floorHtmlWithUnit(nDisplayUnit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, SkicoinUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    {
        // update the display unit, to not use the default ("SKI")
        updateDisplayUnit();
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strPrivateSendAmount = SkicoinUnits::formatHtmlWithUnit(nDisplayUnit, privateSendClient.nPrivateSendAmount * COIN, false, SkicoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strPrivateSendAmount = strPrivateSendAmount.remove(strPrivateSendAmount.indexOf("."), SkicoinUnits::decimals(nDisplayUnit) + 1);
    ...
    if(nMaxToAnonymize >= privateSendClient.nPrivateSendAmount * COIN) {
        ...
        QString strMaxToAnonymize = SkicoinUnits::formatHtmlWithUnit(nDisplayUnit, nMaxToAnonymize, false, SkicoinUnits::separatorAlways);
        ui->labelAmountRounds->setToolTip(tr("Not enough compatible inputs to anonymize <span style='color:red;'>%1</span>,<br>"
                                             "will anonymize <span style='color:red;'>%2</span> instead")
                                          .arg(strPrivateSendAmount)
                                          .arg(strMaxToAnonymize));
        strMaxToAnonymize = strMaxToAnonymize.remove(strMaxToAnonymize.indexOf("."), SkicoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = "<span style='color:red;'>" +
                QString(SkicoinUnits::factor(nDisplayUnit) == 1 ? "" : "~") + strMaxToAnonymize +
...
void OverviewPage::togglePrivateSend(){
    ...
    if(!privateSendClient.fEnablePrivateSend){
        ...
        if(currentBalance < nMinAmount){
            QString strMinAmount(SkicoinUnits::formatWithUnit(nDisplayUnit, nMinAmount));
            QMessageBox::warning(this, tr("PrivateSend"),
