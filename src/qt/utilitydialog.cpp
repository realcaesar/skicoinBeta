    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  skicoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_SKICOIN_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SkicoinGUI::DEFAULT_UIPLATFORM));
        }
...
All the Skicoin in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.1 SKI, 1 SKI, 10 SKI and 100 SKI -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://skicoinpay.atlassian.net/wiki/display/DOC/PrivateSend\">https://skicoinpay.atlassian.net/wiki/display/DOC/PrivateSend</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(SkicoinGUI *window)
{
    if (!window)
        return nullptr;
