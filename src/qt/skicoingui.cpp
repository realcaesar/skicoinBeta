...
#include "config/skicoin-config.h"
#endif

#include "skicoingui.h"
...
const std::string SkicoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SkicoinGUI::DEFAULT_WALLET = "~Default";

SkicoinGUI::SkicoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void SkicoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Skicoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and skicoin: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Skicoin Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Skicoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Skicoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a skicoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Skicoin command-line options").arg(tr(PACKAGE_NAME)));
...
void SkicoinGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if(walletFrame)
    {
...
void SkicoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SkicoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SkicoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SkicoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SkicoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SkicoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void SkicoinGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void SkicoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SkicoinGUI::optionsClicked()
{
...
void SkicoinGUI::showDebugWindow()
{
...
void SkicoinGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void SkicoinGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SkicoinGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void SkicoinGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void SkicoinGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void SkicoinGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void SkicoinGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void SkicoinGUI::showBackups()
{
    GUIUtil::showBackups();
}

void SkicoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void SkicoinGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void SkicoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SkicoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SkicoinGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void SkicoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SkicoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SkicoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SkicoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SkicoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Skicoin network", "", count));
    } else {
...
void SkicoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SkicoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void SkicoinGUI::updateHeadersSyncProgressLabel()
{
...
void SkicoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in skicoin
        {
...
void SkicoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void SkicoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Skicoin Core"); // default title
...
void SkicoinGUI::changeEvent(QEvent *e)
{
...
void SkicoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SkicoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SkicoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SkicoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SkicoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SkicoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SkicoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SkicoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SkicoinGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void SkicoinGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void SkicoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SkicoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SkicoinGUI::detectShutdown()
{
...
void SkicoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SkicoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void SkicoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SkicoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SkicoinGUI::subscribeToCoreSignals()
{
...
void SkicoinGUI::unsubscribeFromCoreSignals()
{
...
void SkicoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SkicoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SkicoinUnits::Unit> units = SkicoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SkicoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SkicoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(SkicoinUnits::Unit u, SkicoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SkicoinUnits::name(u)), this);
...
{
    setText(SkicoinUnits::name(newUnits));
}
