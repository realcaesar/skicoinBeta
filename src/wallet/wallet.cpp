...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CSkicoinAddress(CScriptID(redeemScript)).ToString();
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend, bool fIncludeSkicoinAliasBalances, bool fIncludeSkicoinAliasOutputs) const
{
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
				// SKICOIN
				if (!fIncludeSkicoinAliasBalances || !fIncludeSkicoinAliasOutputs) {
					if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs && !coinControl->IsSelected(COutPoint((*it).first, i)))
						continue;
					// SKICOIN txs are unspendable by wallet unless using coincontrol(and the tx is selected)
					if (!coinControl || !coinControl->IsSelected(COutPoint((*it).first, i)))
					{
						...
						if (pcoin->tx->vout.size() >= i && ExtractDestination(pcoin->tx->vout[i].scriptPubKey, sysdestination))
						{
							...
							if (IsSkicoinScript(pcoin->tx->vout[i].scriptPubKey, op, vvchArgs) && !fIncludeSkicoinAliasOutputs)
								continue;
							CSkicoinAddress address = CSkicoinAddress(sysdestination);
							if (DoesAliasExist(address.ToString()) && !fIncludeSkicoinAliasBalances)
								continue;
						}
					}
				}
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = pcoin->tx->vout[i].nValue == 10500*COIN;
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
	...
	// SKICOIN include skicoin alias balances
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && wtx.tx->vout[i].nValue == 10500*COIN) continue;
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        for (const auto& item : vecTallyRet)
            strMessage += strprintf("  %s %f\n", CSkicoinAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 10500*COIN) continue; //masternode input
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
	...
	// SKICOIN include sys alias balances
...
bool CWallet::GetOutpointAndKeysFromOutput(const COutput& out, COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CSkicoinAddress address2(address1);
...
{
    std::vector<COutput> vCoins;
	// SKICOIN include alias balances
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SKI."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-skicoin-address
                        CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CSkicoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CSkicoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CSkicoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CSkicoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CSkicoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CSkicoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CSkicoinAddress(dest).ToString(), key);
}
