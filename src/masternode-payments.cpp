...
*
*   Why is this needed?
*   - In Skicoin some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Skicoin specific functionality
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
		...
		// SKICOIN update last vote after sig check
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew, const int64_t &nHeight, const CAmount& fee, CAmount& nTotalRewardWithMasternodes) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SKI\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
std::string CMasternodeBlockPayees::GetRequiredPaymentsString() const
{
    ...
    for (const auto& payee : vecPayees)
    {
        ...
        strRequiredPayments += strprintf("%s:%d:%d", address2.ToString(), payee.GetVoteCount(), payee.nStartHeight);
    }
