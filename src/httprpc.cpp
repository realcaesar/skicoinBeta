...
// Skicoin This is not a CORS check. Only check if OPTIONS are
...
static bool checkPreflight(HTTPRequest* req)
{
    ...
    // Skicoin Keep origin so as not to have to set Access-Control-Allow-Origin
    // to *
    ...
    // unbounded.
    // Skicoin We are only satifying the OPTIONS request in this method in order to
    ...
    if (req->GetRequestMethod() == HTTPRequest::OPTIONS) {
        ...
        // Skicoin as we are only handling OPTIONS there is
