...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in skicoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("skicoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Skicoin features */
    { "skicoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "skicoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "skicoin",               "gobject",                &gobject,                true,  {} },
    { "skicoin",               "voteraw",                &voteraw,                true,  {} },
