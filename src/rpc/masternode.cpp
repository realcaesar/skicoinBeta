...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.outpoint.ToStringShort()));
        obj.push_back(Pair("payee",         CSkicoinAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CSkicoinSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CSkicoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Skicoin address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CSkicoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                objMN.push_back(Pair("payee", CSkicoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CSkicoinAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CSkicoinAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CSkicoinAddress(mnb.pubKeyMasternode.GetID()).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "skicoin",               "masternode",             &masternode,             true,  {} },
    { "skicoin",               "masternodelist",         &masternodelist,         true,  {} },
    { "skicoin",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "skicoin",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "skicoin",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "skicoin",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
