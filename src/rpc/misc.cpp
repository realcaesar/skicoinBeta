...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total skicoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized skicoin balance of the wallet\n"
	...
	obj.push_back(Pair("version", SKICOIN_CLIENT_VERSION));
	// SKICOIN
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "skicoin|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "skicoin")
            + HelpExampleRpc("debug", "skicoin+net")
        );
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CSkicoinAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given skicoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The skicoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
			"  \"address\" : \"skicoinaddress\", (string) The skicoin address validated\n"
			// SKICOIN
			"  \"zaddress\" : \"skicoinaddress\", (string) The zcash t-addr associated with this skicoin address validated\n"
			"  \"saddress\" : \"skicoinaddress\", (string) The Skicoin 3 S address scheme associated with this skicoin address validated\n"
			"  \"btcaddress\" : \"skicoinaddress\", (string) The bitcoin address associated with this skicoin address validated\n"
			"  \"alias\" : \"skicoinalias\", (string) The skicoin alias associated with this skicoin address validated\n"
...
#ifdef ENABLE_WALLET
		// Case 1: Skicoin address and we have full public key:
		CSkicoinAddress address(ks);
		// SKICOIN
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are skicoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) skicoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
	...
	addr.Set(dest, CChainParams::ADDRESS_SKI);
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The skicoin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CSkicoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CSkicoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CSkicoinAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CSkicoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "skicoin-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Skicoin features */
    { "skicoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "skicoin",               "spork",                  &spork,                  true,  {"value"} },
