...
const char * const SKICOIN_CONF_FILENAME = "skicoin.conf";
const char * const SKICOIN_PID_FILENAME = "skicoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "skicoin" is a composite category enabling all Skicoin-related debug output
                if(ptrCategory->count(std::string("skicoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SkicoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SkicoinCore
    // Mac: ~/Library/Application Support/SkicoinCore
    // Unix: ~/.skicoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SkicoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SkicoinCore";
#else
    // Unix
    return pathRet / ".skicoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty skicoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override skicoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", SKICOIN_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Skicoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Skicoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The Skicoin Core developers";
    }
