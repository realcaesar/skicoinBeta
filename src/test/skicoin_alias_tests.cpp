// Copyright (c) 2016-2018 The Skicoin Core developers
...
BOOST_FIXTURE_TEST_SUITE (skicoin_alias_tests, BasicSkicoinTestingSetup)
...
BOOST_AUTO_TEST_CASE (generate_big_aliasdata)
{
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "skicointxfund " + varray[0].get_str()));
	varray = r.get_array();
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "signrawtransaction " + varray[0].get_str()));
	BOOST_CHECK_NO_THROW(CallRPC("node1", "skicoinsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	GenerateBlocks(5);
	// activation should fail
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "aliasnew jag2 " + baddata + " 3 0 " + newaddress + " '' '' ''"));
	varray = r.get_array();
	BOOST_CHECK_THROW(r = CallRPC("node1", "skicointxfund " + varray[0].get_str()), runtime_error);
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "skicointxfund " + varray1[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "skicointxfund " + varray3[0].get_str()));
	...
	BOOST_CHECK_NO_THROW(r = CallRPC("node1", "skicointxfund " + varray5[0].get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaswitness)
{
	...
	BOOST_CHECK_NO_THROW(CallRPC("node2", "skicoinsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
...
BOOST_AUTO_TEST_CASE (generate_aliaspay)
{
	...
	for (unsigned int i = 0; i < MAX_ALIAS_UPDATES_PER_BLOCK; i++)
	{
		...
		BOOST_CHECK_NO_THROW(CallRPC("node3", "skicoinsendrawtransaction " + find_value(r.get_obj(), "hex").get_str()));
	}
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithoffer)
{
	...
	string offerguid = OfferNew("node1", "aliasprunewithoffer", "category", "title", "1", "0.05", "description", "SKI");
...
BOOST_AUTO_TEST_CASE (generate_aliasprunewithcertoffer)
{
	...
	string certofferguid = OfferNew("node1", "aliasprunewithcertoffer", "certificates", "title", "1", "0.05", "description", "SKI", certguid);	
	string offerguid = OfferNew("node1", "aliasprunewithcertoffer", "category", "title", "1", "0.05", "description", "SKI");
...
BOOST_AUTO_TEST_CASE (generate_aliasexpired)
{
	...
	CSkicoinAddress aliasAddress(pubKey.GetID());
	...
	BOOST_CHECK_THROW(CallRPC("node2", "offernew aliasexpirednode2 category title 1 0.05 description USD '' SKI false 1 BUYNOW 0 0 false 0 ''"), runtime_error);
	// should fail: new escrow with expired arbiter alias

	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpire2node2 aliasexpirednode2 " + offerguid + " 1 true 0 0 25 0.005 0 '' SKI 0 0 ''"), runtime_error);
	// should fail: new escrow with expired alias
	BOOST_CHECK_THROW(CallRPC("node2", "escrownew false aliasexpirednode2 aliasexpire " + offerguid + " 1 true 0 0 25 0.005 0 '' SKI 0 0 ''"), runtime_error);
	...
	BOOST_CHECK_NO_THROW(CallRPC("node1", "offerupdate aliasexpire0 " + offerguid + " category title 100 0.05 description USD false '' 0 SKI BUYNOW 0 0 false 0 ''"));
	GenerateBlocks(5, "node1");
